...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//위에서의 요청처리 후 성공했다는 가정하에 사용자가 보게된 응답페이지(html) 만들어서 전송
		//즉, 여기 Java코드 내에 사용자가 보게될 응답 html 구문을 작성할 것

		// 장점 : Java코드 내에 작성하기 때문에 반복문이나 조건문, 유용한 메소드 같은걸 활용할 수 있다.
		...
		if(gender == null) {
			...
			if (gender.equals("M")) {
				out.println("<sapn id='gender'>남자</span>입니다.");
			} else {
				out.println("<sapn id='gender'>여자</span>입니다.");
			}
		}
